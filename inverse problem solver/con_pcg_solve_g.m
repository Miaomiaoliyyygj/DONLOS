function x = con_pcg_solve_g(Loss_Sig,xmin,ymin,zmin,xg,yg,zg,kernel,num_measure, N_max, num_grid, C_i, Ask_value, Arr_time,x0, max_iter, b,max_r_tol)
lxs = length(xg); lys = length(yg); lzs = length(zg); l_ker = length(kernel);

nb = norm(b); r_err = NaN * zeros(1,max_iter);
x = x0; r = b - A( x,num_measure,num_grid,kernel,l_ker,...
Ask_value,zmin,zg,ymin,yg,xmin,xg,Arr_time,C_i,lzs,lys,lxs,N_max );

rr = r' * r; p = r; r_err(1) = norm(r)/nb;
if r_err(1) >= max_r_tol
    for k = 2:max_iter   
        Ap = A( p,num_measure,num_grid,kernel,l_ker,...
Ask_value,zmin,zg,ymin,yg,xmin,xg,Arr_time,C_i,lzs,lys,lxs,N_max );
        pAp = p' * Ap;
        alpha = rr / pAp;
        x_new = x + alpha * p;
...
function y = A( x,num_measure,num_grid,kernel,l_ker,Ask_value,zmin,zg,ymin,yg,xmin,xg,Arr_time,C_i,lzs,lys,lxs,N_max )
